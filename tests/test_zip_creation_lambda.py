...
def test_parse_multiple_tags_basic(monkeypatch):
    ...
    module = load_lambda('zip_creation', 'services/zip-processing/src/zip_creation_lambda.py')
...
def test_parse_multiple_tags_missing(monkeypatch):
    _stub_botocore(monkeypatch)
    _stub_defusedxml(monkeypatch)
    module = load_lambda('zip_creation_missing', 'services/zip-processing/src/zip_creation_lambda.py')
