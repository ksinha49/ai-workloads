...
def test_generate_token(monkeypatch):
    ...
    module = load_lambda('tokenize', 'services/anonymization/src/2_tokenize_entities_lambda.py')
...
def test_existing_token(monkeypatch):
    ...
    module = load_lambda('tokenize2', 'services/anonymization/src/2_tokenize_entities_lambda.py')
    out = module.lambda_handler({'entity': 'Bob', 'entity_type': 'NAME', 'domain': 'gen'}, {})
...
def test_uuid_generation(monkeypatch):
    ...
    monkeypatch.delenv('TOKEN_SALT', raising=False)
    module = load_lambda('tokenize3', 'services/anonymization/src/2_tokenize_entities_lambda.py')
